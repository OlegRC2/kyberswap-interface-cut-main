...
import { PoolClassicIcon } from './Icons'
...
function ClassicElasticTab() {
  ...
  const { tab: tabQS = VERSION.CLASSIC, ...qs } = useParsedQueryString<{ tab: string }>()

  const tab = isInEnum(tabQS, VERSION) ? tabQS : VERSION.CLASSIC
